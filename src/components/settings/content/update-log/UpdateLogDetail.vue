      ...
      <el-form ref="form" :rules="rules" :model="updateLogForm" label-width="80px" label-position="left">
        ...
        <el-form-item label="状态" prop="status">
          <el-select v-model="updateLogForm.status" placeholder="请选择状态">
            <el-option v-for="status in updateLogStatus" :key="status.code" :label="status.text" :value="status.code"></el-option>
          </el-select>
        </el-form-item>
      </el-form>
    </el-col>
  ...
  methods: {
    ...
    saveUpdateLog(formName) {
      this.$refs[formName].validate((valid) => {
        ...
        this.updateLogForm.updateDate = this.updateLogForm.createDate = new Date();
